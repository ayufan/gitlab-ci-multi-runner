...
GitLab Runner implements a few shell script generators allowing to execute builds on different systems.

The shell script contains commands to execute all steps of the build:
1. git clone
...
This is the default shell used on all Unix based systems.

The bash script is executed by piping the shell script to one of the following commands:
    ...
    # This command is used if the build should be executed using current user, but in login environment
    cat generated-bash-script | /bin/bash --login

    # This command is used if the build should be executed in Docker environment
    cat generated-bash-script | /bin/bash

## Windows Batch

This is the default shell used on Windows. The Windows Batch doesn't support executing the build in context of another user.

The generated Batch script is executed by saving it content to file and passing the file name to this command:

    cmd /Q /C generated-windows-batch.cmd

This is how the example batch script looks like:
...
The PowerShell doesn't support executing the build in context of another user.

The generated PowerShell script is executed by saving it content to file and passing the file name to this command:
