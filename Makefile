...
darwin-386:
	GOOS=darwin GOARCH=386 bash -c 'go build -o out/gitlab-ci-multirunner-$$GOOS-$$GOARCH$$GOEXT'

darwin:
	GOOS=darwin GOARCH=amd64 bash -c 'go build -o out/gitlab-ci-multirunner-$$GOOS-$$GOARCH$$GOEXT'

windows-386:
	GOOS=windows GOARCH=386 GOEXT=.exe bash -c 'go build -o out/gitlab-ci-multirunner-$$GOOS-$$GOARCH$$GOEXT'
